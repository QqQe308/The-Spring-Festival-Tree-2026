...
addLayer("c", {
  ...
  gainMult() {
    ...
    if (hasUpgrade(this.layer, 120))
      mult = mult.times(upgradeEffect(this.layer, 120));
    return mult;
  },
  gainExp() {
  ...
  effect() {
    return {
      ...
      waffleBoost:
        true == false ? 0 : Decimal.pow(player[this.layer].points, 0.2),
  ...
  effectDescription() {
    ...
    eff = this.effect();
    eff.waffleBoost = eff.waffleBoost.times(
      buyableEffect(this.layer, 11).first,
    );
    return (
      "which are boosting waffles by " +
      format(eff.waffleBoost) +
  ...
  buyables: {
    ...
    11: {
      ...
      buy() {
        ...
        player[this.layer].spentOnBuyables =
          player[this.layer].spentOnBuyables.add(cost); // This is a built-in system that you can use for respeccing but it only works with a single Decimal value
      },
  ...
  layerShown() {
    return true;
  }, // Condition for when layer appears on the tree
  ...
  hotkeys: [
    {
      key: "c",
      description: "C: reset for lollipops or whatever",
      onPress() {
    ...
    {
      key: "ctrl+c",
      description: "Ctrl+c: respec things",
      onPress() {
        respecBuyables(this.layer);
      },
      unlocked() {
      ...
      first: {
        content: [
          ...
          [
            "display-text",
            function () {
              return "confirmed<br>" + player.c.drop;
            },
          ],
  ...
  tabFormat: {
    ...
    jail: {
      ...
      content: [
        ...
        [
          ...
          [
            [
              "column",
              [
                ["display-text", "Sugar level:", { color: "teal" }],
                "blank",
                ["bar", "tallBoi"],
              ],
        ...
        "blank",
        ["display-text", 'It\'s jail because "bars"! So funny! Ha ha!'],
        ["tree", testTree],
      ],
    },
    illuminati: {
      ...
      content: [
        [
          "raw-html",
          function () {
            return "<h1> C O N F I R M E D </h1>";
          },
        ],
        "blank",
        [
  ...
  tooltip() {
    ...
    if (player[this.layer].buyables[11].gt(0))
      tooltip +=
        "<br><i><br><br><br>" +
        formatWhole(player[this.layer].buyables[11]) +
        " Exhancers</i>";
    return tooltip;
  },
  shouldNotify() {
...
const textParticle = {
  ...
  speed: 0,
  text: function () {
    return "<h1 style='color:yellow'>" + format(player.points);
  },
  offset: 30,
  fadeInTime: 1,
};
