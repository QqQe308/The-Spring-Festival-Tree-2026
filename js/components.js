...
function loadVue() {
  ...
  Vue.component("thing-tree", {
    ...
    template: `
 <div>
  <div v-for="(row, r) in data" :key="r" class="upgRow" style="margin-bottom: 50px;">
   <span v-for="id in row" :key="id" 
      v-if="tmp[layer][type+'s'][id] !== undefined && tmp[layer][type+'s'][id].unlocked" 
      class="upgAlign">
    <div :is="type" :layer="layer" :data="id" 
